...
const getHomeInfo = async (ctx: Context): Promise<any> => {
  ...
  try {
    ...
    const province = addressInfo.data.result ? addressInfo.data.result.ad_info.province : '';
    const city = addressInfo.data.result ? addressInfo.data.result.ad_info.city : '';
    const district = addressInfo.data.result ? addressInfo.data.result.ad_info.district : '';
    // 腾讯天气服务，根据城市获取天气预报信息
    ...
    ctx.body = {
      one,
      address: addressInfo.data.result.ad_info,
      wea: wea.data.data
    };
  } catch (err) {
